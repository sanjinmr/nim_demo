...
/**
 * 会话界面（该界面分三部分，即一个纵向布局：网络状态布局、多端登录状态布局、最近会话列表RecentContactsFragment）
...
public class SessionListFragment extends MainTabFragment {
    ...
    private void addRecentContactsFragment() {
        ...
        // UIKit 实现了最近联系人列表界面的事件响应，如果无法满足需求，
        // 开发者也可以通过设置自定义事件回调函数 RecentContactsCallback 来定制。
        fragment.setCallback(new RecentContactsCallback() {
            ...
            public void onItemClick(RecentContact recent) {
                ...
                // 如果需要进入聊天窗口时中显示特定的消息位置，可以调用以下两个方法，新增消息对象参数作为列表显示的位置（根据消息的角色属性决定消息列表的左/右侧）。
