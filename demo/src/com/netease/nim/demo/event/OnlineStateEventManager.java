...
public class OnlineStateEventManager {
    ...
    /**
     * 监听用户在线状态
     * 登录成功后，SDK 会负责维护与服务器的长连接以及断线重连等工作。
     * 当用户在线状态发生改变时，会发出通知。此外，自动登录过程中也会有状态回调。
     * 开发者可以通过加入以下代码监听用户在线状态改变。
     * 被踢出的情况说明：
     当用户在线时被踢出，会立刻收到被踢出的状态变更通知；
     当用户离线后在其他设备成功登录，又在本设备重新自动登录时，也会收到被踢出的状态变更通知。
     开发者也可以主动获取当前用户在线状态：
